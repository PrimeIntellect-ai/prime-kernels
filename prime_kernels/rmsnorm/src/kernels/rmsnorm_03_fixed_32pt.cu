// Small-C variant: two 16-element vectors per thread and no loop over C
...
__global__ __launch_bounds__(BLOCK_SIZE) void fused_rmsnorm_residual_forward_32pt_kernel(
...
{
    fused_rmsnorm_residual_forward_32pt_kernel<BlockSize><<<N, BlockSize, 0, stream>>>(
            vals, scales, rrms, branch, residual, weight, epsilon, N, C, 1.f/C);
}
void launch_rmsnorm_32pt(
        __nv_fp8_e4m3* vals, __nv_fp8_e8m0* scales, float* rrms,
        const nv_bfloat16* branch, const nv_bfloat16* residual,
        const nv_bfloat16* weight,
        float epsilon, int N, int C, cudaStream_t stream)
{
    if (C <= 1024) {
        do_launch_rmsnorm_32pt<32>(vals, scales, rrms, branch, residual, weight, epsilon, N, C, stream);
    } else if (C <= 2048) {
        do_launch_rmsnorm_32pt<64>(vals, scales, rrms, branch, residual, weight, epsilon, N, C, stream);
    } else if (C <= 3072) {
        do_launch_rmsnorm_32pt<96>(vals, scales, rrms, branch, residual, weight, epsilon, N, C, stream);
    } else if (C <= 4096) {
        do_launch_rmsnorm_32pt<128>(vals, scales, rrms, branch, residual, weight, epsilon, N, C, stream);
    } else if (C <= 5120) {
        do_launch_rmsnorm_32pt<160>(vals, scales, rrms, branch, residual, weight, epsilon, N, C, stream);
    } else if (C <= 6144) {
        do_launch_rmsnorm_32pt<192>(vals, scales, rrms, branch, residual, weight, epsilon, N, C, stream);
    } else if (C <= 7168) {
        do_launch_rmsnorm_32pt<224>(vals, scales, rrms, branch, residual, weight, epsilon, N, C, stream);
    } else if (C <= 8192) {
        do_launch_rmsnorm_32pt<256>(vals, scales, rrms, branch, residual, weight, epsilon, N, C, stream);
    } else {
